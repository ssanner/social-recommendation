...
%the first is concerned with the nature and observations on user traits, interactions and diffusion mechanisms;
the first is concerned with correlating these user traits and interactions to user preferences and interests; %and the strength of social ties.
...
The first group of related work tries to correlate from user interactions to preferences and tie strength. 
...
%and such correlation does not hold in one-to-one mode of communicationsuch as email.
...
%to take into account multiple relations~\cite{tf,Jiang2012SRA},  
...
ensemble~\cite{ste}, 
%a low-rank factorization of the social interactions matrix~\cite{sorec},
...
%In summary, our study is motivated by overall utility of diverse and fine-grained user interactions.
To the best of our knowledge, this is the first work on social recommender systems that systematically analyze social affinity using
fine-grained interaction and activities.  
%We show that rich affinity features outperform state-of-the-art
...
%our observations confirm the effect of demising returns on repeated exposure, 
