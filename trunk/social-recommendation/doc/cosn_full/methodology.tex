...
\item \textbf{Activity Social Affinity Groups (ASAGs)}: We define activity affinity groups based on group membership, page likes and user favourites (of which there are over 17000 in our data).  For any one
of these activities $k$ we define:
  \begin{align*}
     \textit{ASAG(k)} := \{ v | \textrm{user $v$ has expressed a preference for $k$} \}
  \end{align*}
...
classes of \emph{social affinity features (SAFs)} that can be used to train
...
$X^{u,i}_k$ for user $u$, item $i$ and interaction
$k \in \textit{Interaction-Affinity-Classes}$ are defined as : We
define interaction affinity features for target user $u$ and item $i$
for ISAG's classes *** $ \langle X_{1},X_{2}\ldots,X_{k}\rangle$ as
  \begin{equation*}
  ...
  \item \textbf{Activity Social Affinity Features (ASAFs)} : We define activity affinity features for target user $u$ and item $i$   \
