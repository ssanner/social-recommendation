...
%%lexing: the following paragraph mixed up argument structure
%%lexing: it says deficiencies but the headings are the main features/improvements being proposed
%%lexing: edited to read as "features"
...
Despite being the state-of-the-art in SCF systems, we notice that
existing SCF MF objective functions can be improved in three key aspects:
%\begin{enumerate}
%\item[(a)] {\bf Feature-based user similarity learning:} 
%%lexing:remove enumeration for better flow
%%lexing:
%%lexing:please double-check the revised statements
(a) Learning user similarity. %Feature-based user similarity learning.
...
%similarity. % based on observed interactions. 
%For example, one may %might 
...
%diffusion:} 
%%lexing:not clear what item (b) meant, please revisit
...
%%lexing:todo -- merge with the above
%%lexing: summary pitch need to be further tuned with complete results
The key algorithmic novelty of this paper is in three aspects:
\begin{itemize}
...
%Among the existing baseline methods, SCF MF is superior to 
%all others including MF.
%the best
%existing baseline%to resolve (a)--(c) 
%outperform a wide range of existing SCF algorithms.
...
%%lexing: add a summary of key contributions??
%%lexing: candidates (1) new objective (2) comprehensive evaluations
...
%%lexing: cut down the long table-of-contents
