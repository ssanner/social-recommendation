...
As illustrated in Fig~\ref{fig:overview}, the high-level objective of this paper is to use 
such social affinity to help decide whether or not a user $u$ will like a digital item $i$ (in our test case, a link). 
We call this recommendation algorithm \textit{Social Affinity Filtering (SAF)}, it infers 
target user $u$'s preference for link $i$ through the preference of their 
...
Although high correlation between \textit{incoming} and \textit{outgoing}
is recently observed~\cite{saez2011high}, but whether or not interaction direction 
affect user preference differently is still an open question. 
...
are analogous to community organizations in the real-world. It allows users to declare membership and supports people to organize activities, to post related content, and to have recurring discussions about them.  Examples of groups include {\em Stanford Thai} (Fig~\ref{fig:overview} bottom left), or {\em Harvard Debate Club}.
  ...
  \item \textbf{Favourites} are analogous to bookmarks (on physical books or on the web browser). It it is a user-created list containing various digital items such as Facebook apps, books, music, and many other types of items to indicate their interest. Example favorite items include {\em Big Bang Theory} (TV series), or {\em FC Barcelona} (soccer club). Fig~\ref{fig:overview} bottom right show a Facebook screenshot when a user adds a favorite.
...
Our evaluation includes thousands of active {\em group}, {\em page} or {\em favorite} features, details of them can be found in Sec~\ref{sec:datadesc}.
...
\begin{itemize}
  ...
  We define 
  \begin{quote}
  \textit{ISAG(u, j)} $:=$ the set of the users who have interaction $j$ with user $u$.
  \end{quote}
   For example,
   \begin{quote}
