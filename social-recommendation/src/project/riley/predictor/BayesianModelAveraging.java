...
public class BayesianModelAveraging extends Predictor {
	...
	public static ArffData getArff(int flag, int friend, String name){
		...
		data.setGroups(((flag == 4 || flag == 8) ? true : false), Launcher.LR_GROUPS_SIZE_OVERRIDE);
		data.setPages(((flag == 5 || flag == 8) ? true : false), Launcher.LR_PAGES_SIZE_OVERRIDE);
		data.setOutgoingMessages(((flag == 6 || flag == 8) ? true : false), Launcher.LR_OUTGOING_SIZE_OVERRIDE);
		data.setIncomingMessages(((flag == 7 || flag == 8) ? true : false), Launcher.LR_INCOMING_SIZE_OVERRIDE);
	...
	public void train() {
		...
		for (int i = 0; i < predictors.length; i++){			
			...
			for (int j = 0; j < Launcher.NUM_FOLDS; j++){
				...
				if (predictors[i] instanceof LogisticRegression){			// populate probabilities for SMB or LR 
					predictors[i].train();	
				...
				for (DataEntry de : predictors[i]._testData._data) {		// calculate weights
		...
		for (int i = 0; i < predictors.length; i++){				// normalise weights
			for (int j = 0; j < Launcher.NUM_FOLDS; j++){
